...
class TwoDOFRobot():
    ...
    def calc_inverse_kinematics(self, EE: EndEffector, soln=0):
        ...
        theta2 = [np.pi - beta, -np.pi + beta]
        alpha_0 = atan2(l2*sin(theta2[0]), l1 + l2*cos(theta2[0]))
        alpha_1 = atan2(l2*sin(theta2[1]), l1 + l2*cos(theta2[1]))
        gamma = atan2(y, x)
        theta1_0 = gamma - alpha_0
        theta1_1 = gamma - alpha_1
        self.theta[0] = theta1_0
        self.theta[1] = theta2[0]
        ########################################
